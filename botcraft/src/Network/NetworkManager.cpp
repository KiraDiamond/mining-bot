...
namespace Botcraft
{
    ...
    void NetworkManager::ProcessPacket(const std::vector<unsigned char>& bytes)
    {
        ...
        if (packet != nullptr)
        {
            // AFK bots do not need entity equipment state, and some 26.2
            // servers send item components here that ProtocolCraft cannot
            // decode yet. Drop this packet before parsing its Slot payload.
            if (packet->GetName() == "Set Equipment")
            {
                return;
            }
            ...
            {
                // AFK clients do not need full item/entity metadata fidelity. 26.2
                // servers can send item components that this ProtocolCraft build
                // does not decode yet, for example full shulker-box item drops.
                // Dropping the single malformed packet is better than aborting
                // the whole AFK connection.
                LOG_WARNING("Skipping unparsable clientbound packet \"" << packet->GetName() << "\"\n" << e.what());
                return;
            }
...
#pragma pop_macro("GetMessage")
    ...
    void NetworkManager::Handle(ClientboundLoginPacket& packet)
    {
        ...
        const bool disable_chat_session = std::getenv("BOTCRAFT_DISABLE_CHAT_SESSION") != nullptr;
        if (authentifier && !disable_chat_session)
        {
